...
the [`run`](/packages/clide-js/docs/modules.md#run) function, which orchestrates
...
The [`run`](/packages/clide-js/docs/modules.md#run) function takes an optional
[configuration object](/packages/clide-js/docs/interfaces/RunOptions.md)
...
[`run`](/packages/clide-js/docs/modules.md#run) function, please refer to the
[Typedoc reference](/packages/clide-js/docs/modules.md#run).
...
[`command`](/packages/clide-js/docs/modules.md#command) factory function. This
...
lives, receives a [`State`](/packages/clide-js/docs/classes/State.md) object.
...
reference](/packages/clide-js/docs/modules.md#command).
...
description) and an [`init`](/packages/clide-js/docs/interfaces/Plugin.md#init)
function. This function is called during the CLI application's initialization
phase and receives the application's
[`Context`](/packages/clide-js/docs/classes/Context.md). The
[`Context`](classes/Context.md) provides access to hooks, commands, and other
...
[`init`](/packages/clide-js/docs/interfaces/Plugin.md#init) function should
...
- [help](/packages/clide-js/src/plugins/help.ts): Adds the `--help`/`-h` option
  and manages printing help messages when the option is present or a
  [`UsageError`](/packages/clide-js/docs/classes/UsageError.md) occurs.
  _Included in the core package._
- [logger](/packages/clide-js/src/plugins/logger.ts): A simple logger that logs
  the result of each execution step. _Included in the core package._
- [command-menu](/packages/clide-plugin-command-menu/): Prompts the user to
...
[`run`](/packages/clide-js/docs/modules.md#run) function, the framework
...
2. **Caller Directory:** If no "commands" directory is found in the current
   ...
   the file that called the [`run`](/packages/clide-js/docs/modules.md#run)
...
- If the file exists, it imports the module and checks for a default export,
  which should be the
  [`CommandModule`](/packages/clide-js/docs/types/CommandModule.html) object.
...
![Basic Command Files](/assets/basic-command-resolution.png)
...
![Directory Files](/assets/directory-resolution.png)
...
![Subcommand Resolution Files](/assets/subcommand-resolution.png)
...
- The module is imported and the `environment` is set to `prod` in the
  [`State.params`](/packages/clide-js/docs/classes/State.md#params).

![Parameterized Command Files](/assets/parameterized-command.png)
...
[`OptionsGetter`](/packages/clide-js/docs/modules.md#optionsgetter) allows
...
[`OptionsGetter`](/packages/clide-js/docs/modules.md#optionsgetter) provides a
...
Visit the [examples](/examples) directory to see Clide-JS in action.

## Reference

Clide-JS uses Typedoc to autogenerate detailed references for each major
component of the framework. See the [Typedoc
reference](/packages/clide-js/docs/modules.html) for a full breakdown of
Clide-JS's APIs.
